...
const Navbar = ({ code, accessToken }) => {
  ...
  return (
    <div className="bg-spotify-1300 md:w-64 text-spotify-100 absolute h-screen top-0 left-0 w-0 flex flex-col">
      ...
      scrollbar scrollbar-thumb-spotify-200">
        {playlists.map((item) => {
          return (
            <Playlists key={item.id} name={item.playlistName} uri={item.uri} />
          );
        })}
      </ul>
