  ...
  return (
    <div
      className={`text-spotify-100 flex h-16 ${bgColor} items-center 
        ease-linear duration-300 fixed md:w-header w-full z-20 top-0 right-0 ${navbar ? 'bg-opacity-100' : 'bg-opacity-0'}`}>
      ...
      {!providerValue?.auth ? (
        ...
        <div className='mr-10 relative'>
          <button className='rounded-full lg:py-1 p-1 bg-spotify flex items-center bg-spotify-1300 hover:bg-spotify-700' onClick={() => setOpen(!open)}>
            <img src={user.avatar} className='rounded-full h-7 lg:mr-2'></img>
            <p className='font-semibold mr-1 hidden lg:block'>{user.username}</p>
