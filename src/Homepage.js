...
const PlaylistItem = ({ item }) => {
  ...
  return (
    <div
      ...
      <p className='text-spotify-text-100 ml-3 font-semibold max-w-[45%] overflow-hidden overflow-ellipsis '>{item.playlistName}</p>
      {visible && <BsPlayCircleFill className='text-6xl hover:text-green-400  text-spotify-400 bg-spotify-1300 rounded-full hover:scale-105 shadow-lg absolute right-5' />}
    </div>
  );
};
  ...
  return (
    <div
      ...
      <div className='text-left'>
        <p className='text-spotify-text-100 mt-3 font-semibold overflow-ellipsis '>{item.name}</p>
        <p className='text-spotify-text-200 max-h-16 text-sm overflow-hidden'>{item.description}</p>
      </div>
      {visible && (
        <BsPlayCircleFill
  ...
  return (
    <div className='h-screen bg-gradient-to-t from-spotify-1300 to bg-blue-700 overflow-x-hidden overflow-y-scroll relative md:ml-64 z-0'>
      ...
      <section className=' px-8 text-spotify-100 mb-10'>
        <h1 className='text-2xl font-semibold mb-2'>Based on your recent listening</h1>
        <h2 className='mb-8 text-spotify-text-200'>Inspired by your recent activity</h2>
